...
from app.models.entities import Vehicle, Incident, VehicleStatus, PriorityLevel, WasteCategory
...
class VehicleAssignmentService:
    ...
    async def rank_candidate_vehicles(
        ...
        stmt = select(Vehicle).where(
            Vehicle.status.in_([VehicleStatus.AVAILABLE, VehicleStatus.ASSIGNED, VehicleStatus.EN_ROUTE, VehicleStatus.COLLECTING])
        )
        ...
        est_weight_kg = (incident.estimated_volume_m3 or 2.0) * 350.0  # Approx 350kg / m³
        ...
        for v in vehicles:
            ...
            dist_m = haversine_distance_meters(v_lat, v_lng, incident.latitude, incident.longitude)
            dist_km = dist_m / 1000.0
            eta_mins = max(3, int(dist_km * 2.5 + 4))  # ~25 km/h urban speed

            # 1. Capacity Factor (0 to 35 points)
            rem_capacity = v.capacity_kg - v.current_load_kg
            ...
            total_score = cap_score + type_score + dist_score - workload_penalty + priority_bonus
            ...
            candidates.append(
                AssignmentCandidate(
                    ...
                    driver_name="Ramesh Patel" if "4402" in v.plate_number else "Vikram Singh",
