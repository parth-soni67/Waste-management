...
async def run_notification_tests():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        assert driver_auth_res.status_code == 200, f"Driver login failed: {driver_auth_res.text}"
        ...
        assert officer_auth_res.status_code == 200, f"Officer login failed: {officer_auth_res.text}"
        ...
        driver_id = driver_user.get("sub") or driver_user.get("user_id") or driver_user.get("id")
        ...
        assign1_res = await client.patch(
            ...
            json={"assigned_driver_id": driver_id, "priority": "P2", "status": "ASSIGNED"},
            headers=officer_headers,
        )
        ...
        assign2_res = await client.patch(
            ...
            json={"assigned_driver_id": driver_id, "priority": "P0", "status": "ASSIGNED"},
            headers=officer_headers,
        )
        ...
        assert driver_notifs_res.status_code == 200, f"Failed to get notifications: {driver_notifs_res.text}"
        notifs_data = driver_notifs_res.json()
        items = notifs_data["items"]
        print(f"   ✅ Driver has {notifs_data['unread_count']} unread notifications (Total: {notifs_data['total_count']})")
        ...
        print(f"   ✅ Verified Critical Notification: '{p0_notif['title']}' (Priority: {p0_notif['priority']})")
        ...
        print(f"   ✅ Verified Standard Notification: '{p2_notif['title']}' (Priority: {p2_notif['priority']})")
        ...
        driver_notifs_res = await client.get(f"{API_BASE}/notifications?limit=10", headers=driver_headers)
        route_notif = next((n for n in driver_notifs_res.json()["items"] if n["notification_type"] == "ROUTE_UPDATED"), None)
        assert route_notif is not None, "Route updated notification not found"
        print(f"   ✅ Verified Route Updated Notification: '{route_notif['title']}' — '{route_notif['message']}'")
        ...
        assert start_res.status_code == 200, f"Start collection failed: {start_res.text}"
        print(f"   ✅ Collection started for {inc1_id}.")

        # Check Officer notifications
        officer_notifs_res = await client.get(f"{API_BASE}/notifications?limit=10", headers=officer_headers)
        start_notif = next((n for n in officer_notifs_res.json()["items"] if n["notification_type"] == "COLLECTION_STARTED" and str(n.get("incident_id")) == str(inc1_id)), None)
        assert start_notif is not None, "Officer did not receive COLLECTION_STARTED notification"
        print(f"   ✅ Officer received notification: '{start_notif['title']}' — '{start_notif['message']}'")
        ...
        data = {"latitude": "23.025", "longitude": "72.578", "notes": "Thoroughly cleaned and disinfected."}
        ...
        print(f"   ✅ Proof uploaded successfully (Verification status: {proof_obj['verification_status']}).")
        ...
        assert complete_res.status_code == 200, f"Complete collection failed: {complete_res.text}"
        ...
        proof_sub_notif = next((n for n in officer_items if n["notification_type"] == "PROOF_SUBMITTED" and str(n.get("incident_id")) == str(inc1_id)), None)
        assert proof_sub_notif is not None, "Officer did not receive PROOF_SUBMITTED notification"
        print(f"   ✅ Officer received notification: '{proof_sub_notif['title']}' — '{proof_sub_notif['message']}'")
        ...
        verified_notif = next((n for n in driver_items if n["notification_type"] == "PROOF_VERIFIED" and str(n.get("incident_id")) == str(inc1_id)), None)
        assert verified_notif is not None, "Driver did not receive PROOF_VERIFIED notification"
        print(f"   ✅ Driver received notification: '{verified_notif['title']}' — '{verified_notif['message']}'")
        ...
        reject_res = await client.post(
            ...
            json={"reason": "Secondary spillage remaining near drainage gate", "notes": "Please clear 5m perimeter"},
            headers=officer_headers,
        )
        ...
        driver_notifs_res = await client.get(f"{API_BASE}/notifications?limit=10", headers=driver_headers)
        rejected_notif = next((n for n in driver_notifs_res.json()["items"] if n["notification_type"] == "PROOF_REJECTED" and str(n.get("incident_id")) == str(inc2_id)), None)
        assert rejected_notif is not None, "Driver did not receive PROOF_REJECTED notification"
        print(f"   ✅ Driver received notification: '{rejected_notif['title']}' — '{rejected_notif['message']}'")

        # -------------------------------------------------------------------
        # TEST 8: Read State Management (Individual Read & Mark All Read)
        # -------------------------------------------------------------------
        print("\n🧪 TEST 8: Read / Unread State Persistence...")
        unread_cnt_before = (await client.get(f"{API_BASE}/notifications/unread-count", headers=driver_headers)).json()["count"]
        ...
        unread_cnt_after = (await client.get(f"{API_BASE}/notifications/unread-count", headers=driver_headers)).json()["count"]
        print(f"   📊 Unread count after reading 1 item: {unread_cnt_after}")
        assert unread_cnt_after == unread_cnt_before - 1

        # Mark all read
        mark_all_res = await client.post(f"{API_BASE}/notifications/read-all", headers=driver_headers)
        assert mark_all_res.status_code == 200

        unread_cnt_final = (await client.get(f"{API_BASE}/notifications/unread-count", headers=driver_headers)).json()["count"]
        ...
        officer_feed = (await client.get(f"{API_BASE}/notifications", headers=officer_headers)).json()["items"]
        officer_user_ids = {n["user_id"] for n in officer_feed}
        assert str(driver_id) not in officer_user_ids, "Security breach: Officer feed contains Driver notifications"

        driver_feed = (await client.get(f"{API_BASE}/notifications", headers=driver_headers)).json()["items"]
        for n in driver_feed:
            assert n["user_id"] == str(driver_id), f"Security breach: Driver feed contains notification for {n['user_id']}"

        print("   ✅ Strict user isolation verified. Driver A cannot see Driver B or Officer notifications.")
